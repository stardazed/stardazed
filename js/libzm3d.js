	...
	lines.forEach(function(line) {
		...
		switch (tokens[0]) {
			case "newmtl":
				...
				break;
			case "Ka":
				curMat[MatProp.AmbientColor] = [parseFloat(tokens[1]), parseFloat(tokens[2]), parseFloat(tokens[3])];
				break;
			case "Kd":
				curMat[MatProp.DiffuseColor] = [parseFloat(tokens[1]), parseFloat(tokens[2]), parseFloat(tokens[3])];
				break;
			case "Ks":
				curMat[MatProp.SpecularColor] = [parseFloat(tokens[1]), parseFloat(tokens[2]), parseFloat(tokens[3])];
				break;
			default:
				break;
		}
...
function parseLWObject(text) {
	var t0 = performance.now();
	var lines = text.split("\n");
	var vv = [], nn = [], tt = [];
	var vertexes = [], normals = [], uvs = [], faces = [];
	var mtlFileName = "";
...
function loadLWMaterialFile(filePath) {
	return loadFile(filePath).then(
		function(text) {
			return parseLWMaterial(text);
		}
	);
}


function loadLWObjectFile(filePath, opts) {
	opts = opts || {};
	...
	return Promise.all([mtlProm, objProm]).then(
		function(values) {
