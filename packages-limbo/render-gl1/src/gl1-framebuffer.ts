...
import { pixelFormatIsDepthStencilFormat, pixelFormatIsDepthFormat, pixelFormatIsStencilFormat } from "@stardazed/pixel-format";
import { FrameBuffer, AttachmentDescriptor, TextureClass } from "@stardazed/render";
...
export function createFrameBuffer(rd: GL1RenderDevice, frameBuffer: FrameBuffer) {
	...
	if (frameBuffer.depthAttachment && frameBuffer.stencilAttachment && (frameBuffer.depthAttachment === frameBuffer.stencilAttachment)) {
		...
		assert(pixelFormatIsDepthStencilFormat(frameBuffer.depthAttachment.texture.pixelFormat));
		attachTexture(rd, GLConst.DEPTH_STENCIL_ATTACHMENT, frameBuffer.depthAttachment);
	}
	else {
		if (frameBuffer.depthAttachment) {
			assert(pixelFormatIsDepthFormat(frameBuffer.depthAttachment.texture.pixelFormat));
			attachTexture(rd, GLConst.DEPTH_ATTACHMENT, frameBuffer.depthAttachment);
		}

		if (frameBuffer.stencilAttachment) {
			assert(pixelFormatIsStencilFormat(frameBuffer.stencilAttachment.texture.pixelFormat));
