...
import { RenderPassDescriptor, ClearMask, DepthTest, FaceCulling, FrontFaceWinding, ScissorRect, Viewport, makeViewport } from "render/renderpass-desc";
...
function glTypeForIndexElementType(rc: RenderContext, iet: IndexElementType): number {
	switch (iet) {
		case IndexElementType.UInt8: return rc.gl.UNSIGNED_BYTE;
		case IndexElementType.UInt16: return rc.gl.UNSIGNED_SHORT;
...
function glTypeForPrimitiveType(rc: RenderContext, pt: PrimitiveType) {
	switch (pt) {
		case PrimitiveType.Point: return rc.gl.POINTS;
		case PrimitiveType.Line: return rc.gl.LINES;
		case PrimitiveType.LineStrip: return rc.gl.LINE_STRIP;
		case PrimitiveType.Triangle: return rc.gl.TRIANGLES;
		case PrimitiveType.TriangleStrip: return rc.gl.TRIANGLE_STRIP;
...
export class RenderPass {
	...
	private mesh_: MeshInstance = 0;

	// TEMPORARY: this class will be broken up, so as of now this stuff is all hacky and deprecated
	// with dependencies up the wazoo

	constructor(private rc: RenderContext, private meshMgr_: MeshManager, private desc_: RenderPassDescriptor, private frameBuffer_: FrameBuffer | null = null) {
	...
	// -- drawing
	drawPrimitives(primitiveType: PrimitiveType, startElement: number, elementCount: number, instanceCount = 1) {
	...
	drawIndexedPrimitives(primitiveType: PrimitiveType, indexElementType: IndexElementType, startElement: number, elementCount: number, instanceCount = 1) {
		const glPrimitiveType = glTypeForPrimitiveType(this.rc, primitiveType);
		const glIndexElementType = glTypeForIndexElementType(this.rc, indexElementType);
		const offsetBytes = startElement * indexElementTypeSizeBytes(indexElementType);
