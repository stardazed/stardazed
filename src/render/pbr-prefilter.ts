...
import { attrPosition2, attrUV2 } from "mesh/layout";
import * as meshgen from "mesh/generate";
import { RenderContext, makeShader } from "render/rendercontext";
import { PixelFormat, glImageFormatForPixelFormat } from "render/pixelformat";
import { Texture, UseMipMaps, makeTexDesc2D, makeTexDescCube } from "render/texture";
import { Pipeline, makePipelineDescriptor } from "render/pipeline";
import { FrameBuffer, makeFrameBufferDescriptor } from "render/framebuffer";
import { ClearMask, DepthTest, makeRenderPassDescriptor, runRenderPass } from "render/renderpass";

import { MeshManager } from "world/mesh";
...
export function prefilteredEnvMap(rc: RenderContext, meshMgr: MeshManager, sourceEnvMap: Texture, numSamples: number) {
	...
	const resultEnvMap = new Texture(rc, resultMapDesc);
	...
	const quad = meshgen.generate(new meshgen.Quad(2, 2), [attrPosition2(), attrUV2()]);
	...
	for (let mip = 0; mip < mipCount; ++mip) {
		...
		levelTextures[mip] = new Texture(rc, levelMapDesc);
