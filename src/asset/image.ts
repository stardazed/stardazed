...
import { BlobReader, FileLoadType, loadFile } from "asset/util";
...
export function loadImageURL(url: URL, mimeType?: string): Promise<ImageData | HTMLImageElement> {
	if (! mimeType) {
		const extension = registry.fileExtensionOfURL(url);
		mimeType = registry.mimeTypeForFileExtension(extension);
	}
	if (! mimeType) {
		return Promise.reject(`Cannot determine mime-type of '${url}'`);
	}

	const loader = registry.urlLoaderForMIMEType(mimeType);
	if (! loader) {
...
export function loadImageFromBuffer(buffer: ArrayBuffer, mimeType: string): Promise<ImageData | HTMLImageElement> {
	const loader = registry.bufferLoaderForMIMEType(mimeType);
	if (! loader) {
		return Promise.reject(`No buffer loader available for mime-type '${mimeType}'`);
	}
	else {
...
function assetGroupForImage(image: ImageData | HTMLImageElement) {
	...
	ag.addTexture({
		name: name,
		descriptor: makeTexDesc2DFromImageSource(image)
	});
...
registry.registerFileExtension("bm", "image/bmp");
registry.registerFileExtension("bmp", "image/bmp");
registry.registerFileExtension("png", "image/png");
registry.registerFileExtension("jpg", "image/jpeg");
registry.registerFileExtension("jpeg", "image/jpeg");
registry.registerFileExtension("gif", "image/gif");

registry.registerLoadersForMIMEType("image/bmp", builtInImageLoader, builtInImageLoader);
registry.registerLoadersForMIMEType("image/png", builtInImageLoader, builtInImageLoader);
registry.registerLoadersForMIMEType("image/jpeg", builtInImageLoader, builtInImageLoader);
registry.registerLoadersForMIMEType("image/gif", builtInImageLoader, builtInImageLoader);
...
registry.registerFileExtension("tga", "image/tga");
registry.registerLoadersForMIMEType("image/tga", tgaLoader, tgaLoader);
