...
import { FBXTextParser } from "./fbx-text";
import { FBXBinaryParser } from "./fbx-binary";

import { loadImageURL, loadImageFromBuffer } from "asset/loader/image";
import { TextureImageSource, makeTexDesc2DFromImageSource } from "render/texture-desc";
...
class FBXDocumentGraph {
	...
	private loadTextures(group: AssetGroup, options: FBXResolveOptions): Promise<AssetGroup> {
		...
		Object.keys(this.videoNodes).forEach((idStr) => {
			...
			const tex: Texture2D = {
				...
				userRef: vidID,
				useMipMaps: options.forceMipMapsOn ? UseMipMaps.Yes : UseMipMaps.No
			};
			...
			for (const c of fbxVideo.children) {
				if (c.name == "UseMipMap") {
					if (! options.forceMipMapsOn) {
						tex.useMipMaps = (<number>c.values[0] != 0) ? UseMipMaps.Yes : UseMipMaps.No;
					}
				}
			...
			const makeTexDesc = (img: TextureImageSource) => {
				return makeTexDesc2DFromImageSource(img, tex.useMipMaps);
			};

			if (fileData) {
				const mime = tex.url ? registry.mimeTypeForURL(tex.url) : "";
				if (! mime) {
		...
		for (const matID in this.materialNodes) {
			...
			for (const c of fbxMat.children) {
				if (c.name === "Diffuse") {
					...
					mat.opacity = clamp01(<number>c.values[0]);
				}
			}
	...
	private makeLayerElementStream(layerElemNode: FBXNode): VertexAttributeStream | null {
		let valueArrayName: string, indexArrayName: string;
		const stream: VertexAttributeStream = {
			name: "",
			includeInMesh: true,
			mapping: VertexAttributeMapping.Undefined
		};
		...
		if (layerElemNode.name == "LayerElementNormal") {
			...
			stream.attr = { role: VertexAttributeRole.Normal, field: VertexField.Floatx3 };
		}
		else if (layerElemNode.name == "LayerElementColor") {
			valueArrayName = "Colors";
			indexArrayName = "ColorIndex";
			stream.attr = { role: VertexAttributeRole.Colour, field: VertexField.Floatx3 };
		}
		else if (layerElemNode.name == "LayerElementUV") {
			valueArrayName = "UV";
			indexArrayName = "UVIndex";
			stream.attr = { role: VertexAttributeRole.UV0 + layerElemIndex, field: VertexField.Floatx2 };
		}
		else if (layerElemNode.name == "LayerElementTangent") {
			valueArrayName = "Tangents";
			indexArrayName = "TangentsIndex";
			stream.attr = { role: VertexAttributeRole.Tangent, field: VertexField.Floatx3 };
		}
			...
			stream.attr = { role: VertexAttributeRole.Material, field: VertexField.SInt32 };
		}
		else {
		...
		for (const c of layerElemNode.children) {
			if (c.name == "Name") {
				...
				if (mappingName == "ByVertice") {
					stream.mapping = VertexAttributeMapping.Vertex;
				}
				else if (mappingName == "ByPolygonVertex") {
					stream.mapping = VertexAttributeMapping.PolygonVertex;
				}
				else if (mappingName == "ByPolygon") {
					stream.mapping = VertexAttributeMapping.Polygon;
				}
				else if (mappingName == "AllSame") {
					stream.mapping = VertexAttributeMapping.SingleValue;
				}
				else {
		...
		if (layerElemNode.name == "LayerElementMaterial") {
			assert(
				stream.mapping == VertexAttributeMapping.Polygon || stream.mapping == VertexAttributeMapping.SingleValue,
	...
	private buildMeshes(group: AssetGroup, _options: FBXResolveOptions) {
		...
		for (const geomID in this.geometryNodes) {
			...
			const mb = new MeshBuilder(positions, null, streams);
		...
		for (const modelID in this.modelNodes) {
			...
			for (const c of fbxModel.children) {
				...
				if (c.name === "Lcl Translation") {
					...
					localRot = quat.fromEuler(deg2rad(vecVal[2]), deg2rad(vecVal[1]),	deg2rad(vecVal[0]));
				}
				else if (c.name === "PreRotation") {
					preRot = quat.fromEuler(deg2rad(vecVal[2]), deg2rad(vecVal[1]), deg2rad(vecVal[0]));
				}
				else if (c.name === "PostRotation") {
					postRot = quat.fromEuler(deg2rad(vecVal[2]), deg2rad(vecVal[1]), deg2rad(vecVal[0]));
				}
			}
...
function parseFBXSource(filePath: string, source: string | ArrayBuffer): Promise<AssetGroup> {
	const t0 = performance.now();
	const del = new FBX7DocumentParser(filePath);
	let parser: parse.FBXParser;
	if (typeof source === "string") {
		parser = new FBXTextParser(source, del);
	}
	else {
		parser = new FBXBinaryParser(source, del);
	}
	parser.parse();
...
registry.registerFileExtension("fbx", "application/fbx");
registry.registerURLLoaderForMIMEType("application/fbx", (url, _) => loadFBXFile(url));
