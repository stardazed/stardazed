...
import { NumericType, UInt8, SInt32, Float, Double } from "core/numeric";
import { TypedArray } from "core/array";
import { convertBytesToString } from "asset/util";
import { FBXBlockAction, FBXParserDelegate, FBXValue, interpretProp70P } from "./fbx-parse";
...
export class FBXBinaryParser {
	...
	private twoExp21_ = Math.pow(2, 21);
	private twoExp32_ = Math.pow(2, 32);
		...
		while (count--) {
			...
			switch (type) {
				...
				case "R":
					...
					if (propLen > 0) {
						const propData = this.bytes_.subarray(this.offset_ + 4, this.offset_ + 4 + propLen);
						if (type == "S") {
