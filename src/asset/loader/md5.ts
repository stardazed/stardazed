...
import { AssetGroup, Transform, Model, SkeletonAnimation, Material, Texture2D, makeMaterial, makeModel, makeTransform } from "asset/types";
import { JointAnimation, TransformAnimationField, TransformAnimationTrack, Mesh, UseMipMaps, MaterialFlags } from "asset/types";
import { loadFile } from "asset/util";
import * as registry from "asset/registry";
import * as parse from "./md5-parse";
...
function constructSkinnedMeshStreams(vertexes: VertexData, weights: WeightData) {
	...
	const streams: VertexAttributeStream[] = [
		{
			name: "weightPos0",
			attr: { field: VertexField.Floatx4, role: VertexAttributeRole.WeightedPos0 },
		...
		{
			name: "weightPos1",
			attr: { field: VertexField.Floatx4, role: VertexAttributeRole.WeightedPos1 },
			mapping: VertexAttributeMapping.Vertex,
			includeInMesh: true,
			values: weightPos1
		},
		{
			name: "weightPos2",
			attr: { field: VertexField.Floatx4, role: VertexAttributeRole.WeightedPos2 },
			mapping: VertexAttributeMapping.Vertex,
			includeInMesh: true,
			values: weightPos2
		},
		{
			name: "weightPos3",
			attr: { field: VertexField.Floatx4, role: VertexAttributeRole.WeightedPos3 },
			mapping: VertexAttributeMapping.Vertex,
			includeInMesh: true,
			values: weightPos3
		},
		{
			name: "jointIndexes",
			attr: { field: VertexField.Floatx4, role: VertexAttributeRole.JointIndexes },
			mapping: VertexAttributeMapping.Vertex,
			includeInMesh: true,
			values: jointIndexes
		}
...
export class MD5MeshBuilder implements parse.MD5MeshDelegate {
	...
	private transformNormalsIntoJointSpace(md: MeshData) {
		const normAttr = md.findFirstAttributeWithRole(VertexAttributeRole.Normal);
		...
		this.textures_.forEach(tex => {
			...
			fileProms.push(
				loadImageURL(tex.url).then(img => {
					tex.descriptor = makeTexDesc2DFromImageSource(img, tex.useMipMaps);
					return tex;
				}).catch(error => {
					console.warn(error);
					return null;
...
function parseMD5MeshSource(filePath: string, source: string): Promise<AssetGroup> {
	const del = new MD5MeshBuilder(filePath);
	const parser = new parse.MD5MeshParser(source, del);
	parser.parse();
	return del.assets();
}
...
function parseMD5AnimSource(filePath: string, source: string): AssetGroup {
	const del = new MD5AnimBuilder(filePath);
	const parser = new parse.MD5AnimParser(source, del);
	parser.parse();
	return del.assets();
}
...
registry.registerFileExtension("md5mesh", "application/idsoftware-md5mesh");
registry.registerFileExtension("md5anim", "application/idsoftware-md5anim");
registry.registerURLLoaderForMIMEType("application/idsoftware-md5mesh", (url, _) => loadMD5Mesh(url));
registry.registerURLLoaderForMIMEType("application/idsoftware-md5anim", (url, _) => loadMD5Anim(url));
