...
import { Pipeline, BlendFactor, BlendOperation, makePipelineDescriptor } from "render/pipeline";
import { RenderPass } from "render/renderpass";
import { Material, MaterialFlags, ShadowType, LightType } from "asset/types";
...
import { MeshManager, MeshInstance, MeshFeatures } from "world/mesh";
import { LightManager, LightInstance, ShadowView, FogDescriptor } from "world/light";
import { SkeletonManager } from "world/skeleton";
import { StdMaterialManager, StdMaterialInstance, StdMaterialArrayView } from "world/stdmaterial";
...
class StdPipeline {
	private cachedPipelines_ = new Map<number, Pipeline>();
	private shadowPipeline_: Pipeline | null = null;
	...
	pipelineForFeatures(feat: number) {
		...
		pld.vertexShader = makeShader(this.rc, gl.VERTEX_SHADER, vertexSource);
		pld.fragmentShader = makeShader(this.rc, gl.FRAGMENT_SHADER, fragmentSource);

		// -- mandatory and optional attribute arrays
		pld.attributeNames.set(VertexAttributeRole.Normal, "vertexNormal");

		if (feat & Features.Skinned) {
			pld.attributeNames.set(VertexAttributeRole.JointIndexes, "vertexJointIndexes");
			pld.attributeNames.set(VertexAttributeRole.WeightedPos0, "vertexWeightedPos0_joint");
			pld.attributeNames.set(VertexAttributeRole.WeightedPos1, "vertexWeightedPos1_joint");
			pld.attributeNames.set(VertexAttributeRole.WeightedPos2, "vertexWeightedPos2_joint");
			pld.attributeNames.set(VertexAttributeRole.WeightedPos3, "vertexWeightedPos3_joint");
		}
		else {
			pld.attributeNames.set(VertexAttributeRole.Position, "vertexPos_model");
		}
		if (feat & Features.VtxColour) {
			pld.attributeNames.set(VertexAttributeRole.Colour, "vertexColour");
		}
		if (feat & Features.VtxUV) {
			pld.attributeNames.set(VertexAttributeRole.UV, "vertexUV");
		}
		...
		if (feat & Features.Translucency) {
			...
			pld.blending.rgbBlendOp = BlendOperation.Add;
			pld.blending.alphaBlendOp = BlendOperation.Add;

			if (feat & Features.DiffuseAlphaIsOpacity) {
				pld.blending.sourceRGBFactor = BlendFactor.SourceAlpha;
				pld.blending.sourceAlphaFactor = BlendFactor.SourceAlpha;
				pld.blending.destRGBFactor = BlendFactor.OneMinusSourceAlpha;
				pld.blending.destAlphaFactor = BlendFactor.OneMinusSourceAlpha;
			}
			else {
				// fixed alpha value from Material
				pld.blending.sourceRGBFactor = BlendFactor.ConstantAlpha;
				pld.blending.sourceAlphaFactor = BlendFactor.ConstantAlpha;
				pld.blending.destRGBFactor = BlendFactor.OneMinusConstantAlpha;
				pld.blending.destAlphaFactor = BlendFactor.OneMinusConstantAlpha;

				pld.blending.constantColour[3] = 0.35;
			}
		}

		const pipeline = new Pipeline(this.rc, pld);
	...
	shadowPipeline() {
		if (this.shadowPipeline_ == null) {
			const pld = makePipelineDescriptor();
			pld.depthPixelFormat = PixelFormat.Depth24I;
			pld.vertexShader = makeShader(this.rc, this.rc.gl.VERTEX_SHADER, this.shadowVertexSource);
			pld.fragmentShader = makeShader(this.rc, this.rc.gl.FRAGMENT_SHADER, this.shadowFragmentSource);
			pld.attributeNames.set(VertexAttributeRole.Position, "vertexPos_model");
			// pld.writeMask.red = pld.writeMask.green = pld.writeMask.blue = pld.writeMask.alpha = false;

			this.shadowPipeline_ = new Pipeline(this.rc, pld);
...
export class StdModelManager implements ComponentManager<StdModelManager> {
	...
	)
	{
		...
		const groupFields: MABField[] = [
			{ type: SInt32, count: 1 }, // material
			{ type: SInt32, count: 1 }, // features
		];
		this.primGroupData_ = new MultiArrayBuffer(2048, groupFields);
	...
	private featuresForMeshAndMaterial(mesh: MeshInstance, material: StdMaterialInstance): Features {
		...
		if (matFlags & MaterialFlags.usesSpecular) { features |= Features.Specular; }
		if (matFlags & MaterialFlags.usesEmissive) { features |= Features.Emissive; }
		if (matFlags & MaterialFlags.diffuseAlphaIsTransparency) { features |= Features.DiffuseAlphaIsTransparency; }

		if (matFlags & MaterialFlags.isTranslucent) {
			features |= Features.Translucency;

			if (matFlags & MaterialFlags.diffuseAlphaIsOpacity) {
		...
		if (this.materialMgr_.flags(material) & MaterialFlags.isSkinned) { features |= Features.Skinned; }
	...
	private updatePrimGroups(modelIx: number) {
		...
		if (this.primGroupData_.resize(primGroupCount + groups.length) == InvalidatePointers.Yes) {
			this.groupRebase();
		}
	...
	private drawSingleForward(rp: RenderPass, proj: ProjectionSetup, shadow: ShadowView | null, fogSpec: FogDescriptor | null, modelIx: number) {
		const gl = this.rc.gl;
		let drawCalls = 0;
		...
		for (let pgIx = 0; pgIx < primGroupCount; ++pgIx) {
			...
			if (indexElementType !== IndexElementType.None) {
	...
	private drawSingleShadow(rp: RenderPass, proj: ProjectionSetup, shadowPipeline: Pipeline, modelIx: number) {
		const gl = this.rc.gl;
		...
		if (uniformPrimType !== PrimitiveType.None) {
	...
	draw(range: StdModelRange, rp: RenderPass, proj: ProjectionSetup, shadow: ShadowView | null, fogSpec: FogDescriptor | null, mode: RenderMode) {
		let drawCalls = 0;
