...
export class StdMaterialManager implements ComponentManager<StdMaterialManager> {
	...
	private diffuseMaps_: (Texture | null)[] = [];
	private specularMaps_: (Texture | null)[] = [];
	private normalMaps_: (Texture | null)[] = [];
	...
	private flagsBase_: ConstEnumArrayView<MaterialFlags>;

	private tempVec4 = new Float32Array(4);

	private assetMaterialMap_: WeakMap<Material, StdMaterialInstance>;
	...
	constructor() {
		...
		this.assetMaterialMap_ = new WeakMap<Material, StdMaterialInstance>();
	}
	...
	create(desc: Material): StdMaterialInstance {
	...
	diffuseMap(inst: StdMaterialInstance): Texture | null {
		return this.diffuseMaps_[<number>inst];
	}

	setDiffuseMap(inst: StdMaterialInstance, newTex: Texture) {
		this.diffuseMaps_[<number>inst] = newTex;
	}


	specularMap(inst: StdMaterialInstance): Texture | null {
		return this.specularMaps_[<number>inst];
	}

	setSpecularMap(inst: StdMaterialInstance, newTex: Texture) {
		this.specularMaps_[<number>inst] = newTex;
	}


	normalMap(inst: StdMaterialInstance): Texture | null {
		return this.normalMaps_[<number>inst];
	}

	setNormalMap(inst: StdMaterialInstance, newTex: Texture) {
