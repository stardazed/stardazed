...
import { VertexAttributeRole, IndexElementType } from "mesh/meshdata";
import { RenderContext, makeShader } from "render/rendercontext";
import { PixelFormat } from "render/pixelformat";
import { Texture, TextureRepeatMode, makeTexDesc2DFromImageSource } from "render/texture";
import { Pipeline, makePipelineDescriptor } from "render/pipeline";
import { RenderPass } from "render/renderpass";
import { Material, LightType, UseMipMaps } from "asset/types";
...
import { RenderFeature } from "world/stdmodel";
import { PBRMaterialManager, PBRMaterialFlags, PBRMaterialInstance, PBRMaterialArrayView } from "world/pbrmaterial";
...
export class PBRModelManager implements ComponentManager<PBRModelManager> {
	...
	private brdfLookupTex_: Texture | null = null;
	...
	private loadBRDFLUTTexture() {
		const img = new Image();
		img.onload = () => {
			const td = makeTexDesc2DFromImageSource(img, UseMipMaps.No);
			td.sampling.repeatS = TextureRepeatMode.ClampToEdge;
			td.sampling.repeatT = TextureRepeatMode.ClampToEdge;
			this.brdfLookupTex_ = new Texture(this.rc, td);
		};
		img.onerror = (ev) => {
	...
	private drawSingleForward(rp: RenderPass, proj: ProjectionSetup, lightingQuality: PBRLightingQuality, modelIx: number) {
		const gl = this.rc.gl;
		let drawCalls = 0;
	...
	draw(range: PBRModelRange, rp: RenderPass, proj: ProjectionSetup, lightingQuality: PBRLightingQuality, environmentMap: Texture) {
