...
import { RenderDevice, RenderCommand, RenderCommandBuffer, ResourceType, Sampler, Texture, FrameBuffer, Shader, Mesh, ResourceCommand } from "@stardazed/render";
import { GLConst } from "./gl1-constants";
import { GL1State } from "./gl1-state";
import { ReusableResourceArray } from "./gl1-resource-array";
...
export class GL1RenderDevice implements RenderDevice {
		...
		for (const resource of rrc.free) {
			...
			switch (resource.renderResourceType) {
				...
				case ResourceType.Mesh:
					this.freeMesh(resource as Mesh);
					break;
				default:
					break;
			}
		...
		for (const resource of rrc.alloc) {
			...
			switch (resource.renderResourceType) {
				...
				case ResourceType.Shader:
					...
					break;
				case ResourceType.Mesh:
					this.allocMesh(resource as Mesh);
					break;
				default:
					break;
			}
	...
	readonly meshes_ = new ReusableResourceArray<Geometry, GL1MeshData>(ResourceType.Mesh);

	private allocMesh(geom: Geometry) {
		const gpuMesh = createMesh(this, geom);
		this.meshes_.insert(geom, gpuMesh);
	}

	private freeMesh(geom: Geometry) {
